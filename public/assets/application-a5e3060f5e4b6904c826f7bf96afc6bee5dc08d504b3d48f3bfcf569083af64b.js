...
(function() {
  ...
  ready = function() {
    ...
    $('[data-timeout]').each(function() {
      ...
      $timeout = $(this);
      return setTimeout(function() {
        if ($timeout.is('.alert-box')) {
          return $timeout.find("a.close").trigger("click.fndtn.alert");
        } else {
    ...
    return $expandables.click(function(event) {
      var clicked;
      if (!$(event.target).is('a')) {
        event.preventDefault();
        clicked = this;
        return $expandables.each(function() {
          var $expand, $expandable, shouldExpand;
          $expandable = $(this);
          $expand = $expandable.find('.expand');
          shouldExpand = this === clicked && !$expandable.hasClass('expanded');
          $expandable.toggleClass('expanded', shouldExpand);
          if (shouldExpand) {
            return $expand.slideDown(200);
          } else {
            return $expand.slideUp(200);
          }
        });
      }
    });
